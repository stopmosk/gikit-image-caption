...
class BaseTrainer:
    ...
    def _inference_run(self, dataset_type):
        if self.config.training_parameters.evalai_inference is True:
            ...
            # self.predict_live(dataset_type)
            return
    ...
    def predict_live(self, sample):
        reporter = self.dataset_loader.get_test_reporter('test')
        with torch.no_grad():
            self.model.eval()

            reporter.next_dataset()
            dataloader = reporter.get_dataloader()
            dataset = reporter.current_dataset

            assert len(dataloader) == 1
            batch = next(iter(dataloader))

            s = sample
            # image_feature_0  -- object feats
            # image_feature_1  -- OCR feats

            num_boxes = s['num_boxes']
            num_ocr = len(s['ocr_tokens'])

            obj_bbox_coordinates = dataset.copy_processor({'blob': s['obj_normalized_boxes']})['blob']
            obj_features = dataset.copy_processor({'blob': s['features']})['blob']

            # Preprocess OCR tokens
            ocr_max_len = 50
            ocr_tokens = [
                dataset.ocr_token_processor({'text': token})['text']
                for token in s['ocr_tokens']
            ]
            # Get FastText embeddings for OCR tokens
            context = dataset.context_processor({'tokens': ocr_tokens})
            # Get PHOC embeddings for OCR tokens
            context_phoc = dataset.phoc_processor({'tokens': ocr_tokens})
            # OCR order vectors
            order_vectors = np.eye(len(context['tokens']), dtype=np.float32)
            order_vectors = torch.from_numpy(order_vectors)
            order_vectors[context['length']:] = 0
            # OCR bounding box information
            ocr_bbox_coordinates = dataset.copy_processor({'blob': s['ocr_normalized_boxes']})['blob'][:ocr_max_len]
            ocr_confidence = dataset.copy_processor({'blob': np.expand_dims(np.array(s['ocr_confidence'], dtype='float32'), 1)})['blob'][:ocr_max_len]

            im_ocr_feats = torch.zeros((1, ocr_max_len, 2048))
            im_ocr_feats[:1, :num_ocr] = torch.as_tensor(s['ocr_features'])

            b = batch  #dict()

            b['image_id'] = [s['image_id']]

            b['obj_bbox_coordinates'] = obj_bbox_coordinates.unsqueeze(0)
            b['image_feature_0'] = obj_features.unsqueeze(0)  # 1 x 150 x 2048  obj feats
            b['image_info_0'] = SampleList([('max_features', torch.tensor([num_boxes]))])

            b['context'] = context['text'].unsqueeze(0)  # Fasttext 1x50x300 
            b['context_tokens'] = [context['tokens']]  # 1 x 4094
            b['context_tokens_enc'] = [enc_obj2bytes(context['tokens'])]
            b['context_feature_0'] = context['text'].unsqueeze(0)  #fasttext same
            b['context_info_0'] = SampleList([('max_features', torch.tensor([num_ocr]))])

            b['context_feature_1'] = context_phoc['text'].unsqueeze(0)  # PHOC 1x50x604
            b['context_info_1'] = SampleList([('max_features', torch.tensor([num_ocr]))])

            b['order_vectors'] = order_vectors.unsqueeze(0)  # 1 x 50 x 50 ocr

            b['ocr_bbox_coordinates'] = ocr_bbox_coordinates.unsqueeze(0)
            b['ocr_confidence'] = ocr_confidence.unsqueeze(0)

            b['ocr_tokens'] = [ocr_tokens[:ocr_max_len]]

            b['image_feature_1'] = im_ocr_feats  # 1 x 150 x 2048 ocr feats
            b['image_info_1'] = SampleList([('max_features', torch.tensor([num_ocr]))])

            prepared_batch = reporter.prepare_batch(batch)
            model_output = self.model(prepared_batch)
            report = Report(prepared_batch, model_output)

            gen_result = reporter.add_to_report(report)[0]
            # print(gen_result)
