...
class CaptionTensorizerOCR(object):
    def __init__(self,
        ...
        # for OCR
        model_file = '../pretrained_models/wiki.en.bin'
        fasttext_model = load_model(model_file)
        # String to Vector
        self.str2vec = WordToVectorDict(fasttext_model)
        self.ft_pad = self.str2vec[' ']
    ...
    def tensorize_example_v2(self, text_a, img_feat, text_b=None, text_c=None,
        ...
        if text_c is not None:
            tokens_c = [self.str2vec[text_block] for text_block in text_c]
            tokens_c_pos = text_c_pos
            ...
            ocr_tokens += [self.str2vec[' ']] * padding_c_len  # [OCR, [PAD]s] = 50
            tokens_c_pos.extend([[0, 0, 0, 0, 0, 0]] * padding_c_len)  # PAD to 50
            input_ocr_ids = ocr_tokens
            #print(input_ocr_ids, flush=True)
        else:
            ocr_tokens = [self.ft_pad] * self.max_ocr_seq_length  # [[PAD]s] = 50
            input_ocr_ids = ocr_tokens
            # print(input_ocr_ids, flush=True)
    ...
    for epoch in range(int(args.num_train_epochs)):
        for step, (img_keys, batch) in enumerate(train_dataloader):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if step > 100:   # Because of UserWarning: Detected call of scheduler.step() before optimizer.step()
                    scheduler.step()
